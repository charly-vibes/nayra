...
import { determineLOD, filterEventsByLOD, LOD_MESO, LOD_MICRO } from '../../src/rendering/lod.js';
import { DATASET_BASE_TIME, getDataset1K, getDataset10K, getDataset50K } from './fixtures/generate.js';
...
function getEventBounds(event, viewportStart, scale, layouts) {
  ...
  const endX = event.end !== undefined ? projectToScreen(event.end, viewportStart, scale) : x + 4;
  ...
  describe('Initial Render Pipeline', () => {
    ...
    for (const { label, getData, layoutBudget, hashBudget, pipelineBudget } of SIZES) {
      ...
      it(`spatial hash rebuild — ${label} events completes within ${hashBudget}ms (mean)`, () => {
        ...
        const stats = bench(() => hash.rebuild(events, (e) => getEventBounds(e, VIEWPORT_START, SCALE, layouts)), {
          warmup,
          iterations: iters,
        });
        ...
        console.log(
          `SpatialHash.rebuild(${label}): mean=${stats.mean.toFixed(2)}ms ` + `p95=${stats.p95.toFixed(2)}ms`,
        );
      });
      ...
      it(`full pipeline — ${label} events completes within ${pipelineBudget}ms (mean)`, () => {
        ...
        console.log(`Full pipeline (${label}): mean=${stats.mean.toFixed(2)}ms ` + `p95=${stats.p95.toFixed(2)}ms`);
      });
    }
  });
  ...
  describe('Pan Operations', () => {
    ...
    for (const { label, getData, frames, budgetMs, runs } of PAN_SIZES) {
      it(`${frames}-frame pan sequence — ${label} events within ${budgetMs}ms total`, () => {
        ...
        console.log(`Pan ${frames}f (${label}): ${stats.mean.toFixed(0)}ms total, ` + `${perFrame.toFixed(2)}ms/frame`);
      });
    }
  });
  ...
  describe('Zoom Operations', () => {
    ...
    for (const { label, getData, budget, iters } of LOD_SIZES) {
      it(`filterEventsByLOD (MESO) — ${label} events within ${budget}ms (mean)`, () => {
        ...
        const stats = bench(() => filterEventsByLOD(events, LOD_MESO), { warmup: 5, iterations: iters });
