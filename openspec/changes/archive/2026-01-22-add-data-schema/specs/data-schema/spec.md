...
The data schema defines the JSON format for importing timeline events into Nayra. The schema prioritizes human readability and compatibility with common data sources like Wikidata, RDF knowledge graphs, and hand-authored datasets. Events can span any timescale from recent history to deep cosmological time.
...
- Time strings are parsed to BigInt coordinates internally (see `time-coordinates` spec)
- The JSON Schema file (`schemas/event.schema.json`) provides machine-readable validation
- Wikidata transformation can be done client-side or via a build-time script
- Large datasets (>10,000 events) may benefit from pre-processing at build time
- **Localization:** `label` and `description` are single-language strings. Multilingual support (e.g., `labels: { "en": "...", "es": "..." }`) is out of scope for initial implementation but may be added later

## Related

- `data-loading`: How data files are fetched and cached
- `time-coordinates`: How time is represented internally (BigInt)
- `time-parser`: Parsing implementation for time strings
